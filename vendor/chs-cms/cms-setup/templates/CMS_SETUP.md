...
This site uses the `@chester-hill-solutions/cms-*` packages: a block-based page builder backed by
...
Merge `wrangler-snippet.jsonc` (from `cms-setup wrangler` or
`node_modules/@chester-hill-solutions/cms-setup/templates/`) into your `wrangler.jsonc`. The CMS expects:
...
The `cms-setup init` command copied the CMS migrations into `migrations/`.
...
- import `@chester-hill-solutions/cms-react/styles/cms-tokens.css` for the defaults, or
- define the `--ink`, `--ink-muted`, `--accent-*`, `--surface-*` variables in your own theme.

Add the package to Tailwind content scanning in your main CSS file:

```css
@source "../../node_modules/@chester-hill-solutions/cms-react/src";
```

Install packages from GitHub Packages — see
[docs/USING-PACKAGES.md](https://github.com/chester-hill-solutions/chester-hill-solutions/blob/main/docs/USING-PACKAGES.md).
