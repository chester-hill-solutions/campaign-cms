...
const BLOCK_TYPE_HINTS: Partial<Record<BlockType, string>> = {
...
const GROUPS: ReadonlyArray<{ label: string; types: readonly BlockType[] }> = [
...
export function InsertMenu({ onPick, onClose }: Props) {
  ...
  const flatTypes = useMemo(
    () => filteredGroups.flatMap((group) => group.types),
    [filteredGroups],
  )
  ...
  return (
    <>
      <div
        ...
        onClick={onClose}
      />
      {/* eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-noninteractive-element-interactions -- stops bubbling to canvas deselect only */}
      <div
        className="pb-insert-menu absolute left-1/2 top-full z-50 mt-1 w-80 -translate-x-1/2 overflow-hidden rounded-xl border border-border-subtle bg-surface-card shadow-xl"
        role="dialog"
      ...
      >
        ...
        <div ref={listRef} className="max-h-72 overflow-y-auto p-1.5">
          {flatTypes.length === 0 ? (
            <p className="m-0 px-2 py-3 text-sm text-ink-muted">No blocks match.</p>
          ) : (
            filteredGroups.map((group) => (
              <div key={group.label}>
                <p className="m-0 px-2 pb-1 pt-2 text-[0.65rem] font-semibold uppercase tracking-wide text-ink-muted">
                  {group.label}
                </p>
                {group.types.map((type) => {
                  ...
                  return (
                    <button
                      ...
                      className={`focus-ring flex w-full items-center gap-2.5 rounded-lg px-2 py-1.5 text-left ${
                        isActive ? 'bg-surface-elevated' : 'hover:bg-surface-elevated'
                      }`}
                      onMouseEnter={() => setActiveIndex(flatTypes.indexOf(type))}
                      onClick={() => onPick(type)}
                    >
                      <span className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-md border border-border-subtle bg-surface-page text-ink-muted">
