...
export async function loadDraftForEdit(
  store: CmsAgentStore,
  entryId: string,
): Promise<ToolResult<DraftForEdit>> {
  ...
  if (!base) {
    return toolError(
      'PRECONDITION',
      `Entry "${entryId}" has no draft or published content to edit`,
      { suggestion: 'Use create_page to start a new page.' },
    )
  }
...
export function checkExpectedRevision(
  ...
  expectedDraftRevisionId: string | undefined,
): ToolResult<null> {
  if (
...
export async function validateAndSaveDraft(
  ...
  content: PageMeta,
  message: string,
): Promise<ToolResult<{ entryId: string }>> {
  ...
  if (!saved.ok) {
    return toolError('VALIDATION', 'Draft was rejected by the store', {
      fieldErrors: Object.entries(saved.fieldErrors).flatMap(([field, messages]) =>
 ...
 */
export async function applyDraftMutation<TInfo = undefined>(
  store: CmsAgentStore,
  entryId: string,
  expectedDraftRevisionId: string | undefined,
  message: string,
  mutate: (content: PageMeta) => ToolResult<{ content: PageMeta; info: TInfo }>,
): Promise<ToolResult<{ entryId: string; info: TInfo }>> {
  const loaded = await loadDraftForEdit(store, entryId)
  if (!loaded.ok) return loaded

  const revisionCheck = checkExpectedRevision(
    loaded.data.draftRevisionId,
  ...
  const saved = await validateAndSaveDraft(
    store,
    entryId,
    mutated.data.content,
    message,
  )
