...
const LAYOUT_TYPE_SET: ReadonlySet<string> = new Set(LAYOUT_BLOCK_TYPES)
const APP_TYPE_SET: ReadonlySet<string> = new Set(APP_BLOCK_TYPES)
...
function summarizeBlock(block: PageBlock): {
  ...
  const probe = block as Record<string, unknown>
  const candidate =
    probe.heading ?? probe.headline ?? probe.title ?? probe.text ?? probe.markdown
  const summary =
...
export function createReadTools(
  ...
  return [
    makeTool({
      name: 'list_pages',
      description:
        'List all CMS pages with entry id, slug, title, and publish status.',
    ...
    makeTool({
      name: 'get_page',
      description:
        'Get a page by entry id. view: "draft", "published", "both" (default), or "summary" (meta plus a compact block list — use for large pages).',
    ...
    makeTool({
      ...
      description:
        'List every block type with its category, label, and editable fields (derived from the schemas). All blocks also share id, type, and layout (width/background/spacing/align). Call this before add_block or update_block.',
    ...
    makeTool({
      name: 'list_revisions',
      description:
        'List the revision history (id, version, created_at, message) for an entry, newest first.',
    ...
    makeTool({
      name: 'list_media',
      description:
        'List uploaded media assets (newest first). Use the returned r2_key as /media/{r2_key} in image block src fields.',
    ...
    makeTool({
      name: 'check_slug',
      description:
        'Check whether a slug is valid and available for a new page before calling create_page.',
