...
export function createBlockTools(
  ...
  return [
    makeTool({
      name: 'add_block',
      description:
        'Insert a new block of the given type into the page draft. index defaults to the end. Returns the new block id; use update_block to fill in content.',
      inputSchema: addBlockInput,
      mutates: true,
      config,
      handler: ({ entryId, expectedDraftRevisionId, type, index }) =>
        applyDraftMutation(
          store,
          entryId,
          expectedDraftRevisionId,
          `agent:add_block ${type}`,
          (content) => {
            const insertIndex = index ?? content.blocks.length
            const { content: next, block } = insertBlockAt(
              content,
              type,
              insertIndex,
            )
    ...
    makeTool({
      name: 'update_block',
      description:
        'Merge partial fields into an existing block in the page draft. Field names must match the block schema (see get_block_types). id and type cannot be changed.',
      inputSchema: updateBlockInput,
      mutates: true,
      config,
      handler: ({ entryId, expectedDraftRevisionId, blockId, patch }) =>
        applyDraftMutation(
          store,
          entryId,
          expectedDraftRevisionId,
          ...
          (content) => {
            ...
            if ('type' in patch && patch.type !== existing.type) {
              return toolError(
                'VALIDATION',
                'Block type cannot be changed — remove the block and add a new one',
              )
            }
            ...
            if (!parsed.success) {
              const fieldErrors: ToolFieldError[] = parsed.error.issues.map(
                (issue) => ({
                  blockId,
              ...
              return toolError(
                'VALIDATION',
                `Invalid fields for block type "${existing.type}"`,
                {
                  fieldErrors,
                  suggestion:
                    'Call get_block_types to see the valid fields for this block type.',
                },
              )
            }
    ...
    makeTool({
      ...
      mutates: true,
      config,
      handler: ({ entryId, expectedDraftRevisionId, blockId }) =>
        applyDraftMutation(
          store,
          entryId,
          expectedDraftRevisionId,
          ...
          (content) => {
            ...
            if (content.blocks.length === 1) {
              return toolError(
                'PRECONDITION',
                'Cannot remove the last block — pages need at least one block',
                { suggestion: 'Add a replacement block first.' },
              )
            }
    ...
    makeTool({
      name: 'reorder_block',
      description:
        'Move a block within the page draft. Provide either { blockId, direction: "up"|"down" } or { fromIndex, toIndex }.',
          ...
          (content) => {
            ...
            if (input.blockId !== undefined && input.direction !== undefined) {
              ...
              if (toIndex < 0 || toIndex >= content.blocks.length) {
                return toolError(
                  'PRECONDITION',
                  `Block is already at the ${input.direction === 'up' ? 'top' : 'bottom'} of the page`,
                )
              }
            } else {
              ...
              if (fromIndex > max || toIndex > max) {
                return toolError(
                  'VALIDATION',
                  `Index out of range — the page has ${content.blocks.length} blocks`,
                )
              }
            }
    ...
    makeTool({
      ...
      description:
        'Duplicate a block in the page draft. The copy is inserted directly after the original; returns the new block id.',
        ...
        applyDraftMutation(
          ...
          (content) => {
            ...
            if (content.blocks.length >= 100) {
              return toolError(
                'PRECONDITION',
                'Page is at the 100-block limit',
              )
            }
