...
export type ToolErrorCode =
  | 'NOT_FOUND' // entry, block, revision, or media asset missing
  | 'VALIDATION' // tool input or pageDoc schema failed
  | 'CONFLICT' // slug taken, optimistic revision mismatch
  | 'FORBIDDEN' // assertCanRead / assertCanMutate denied
  | 'PRECONDITION' // e.g. publish with no draft
  | 'INTERNAL' // unexpected; retryable

export type ToolFieldError = { blockId?: string; field: string; message: string }
...
export type ToolResult<T = unknown> = { ok: true; data: T } | ToolError
...
export type CmsAgentConfig = {
  ...
  newPageDoc: (input: { title: string; slug: string }) => PageMeta
...
export type CmsAgentStore = {
  ...
  getPageDocEditorState: (
    entryId: string,
  ) => Promise<EntryEditorState<PageDocPayload> | null>
  ...
  publishEntry: (entryId: string) => Promise<{ published: boolean }>
